...
export function assertComposeDatabaseTarget(
  ...
  const expectedUser = env.POSTGRES_USER?.trim() || "noviqwiki";
  const expectedDatabase = env.POSTGRES_DB?.trim() || "noviqwiki";
...
export function assertRestoreConfirmation(
  ...
  if (confirmation !== expected) {
    throw new Error(
      `Set NOVIQWIKI_RESTORE_CONFIRM=${expected} to confirm the exact destructive restore target.`
    );
  }
}
...
export async function prepareSafeBackupDirectory(backupDirectory: string) {
  ...
  try {
    ...
    if (existing.isSymbolicLink() || !existing.isDirectory()) {
      throw new Error("NOVIQWIKI_BACKUP_DIR must be a real directory, not a link or file.");
    }
    const canonical = await realpath(resolved);
    assertDedicatedBackupPath(canonical);
    assertOwnedByCurrentUser(existing, "NOVIQWIKI_BACKUP_DIR");
    if ((existing.mode & 0o077) !== 0) {
      throw new Error(
        "An existing NOVIQWIKI_BACKUP_DIR must already be private (mode 0700 or stricter)."
      );
    }
    return canonical;
  } catch (error) {
  ...
  if (!created.isDirectory() || created.isSymbolicLink()) {
    throw new Error("NOVIQWIKI_BACKUP_DIR could not be created as a private real directory.");
  }
  assertOwnedByCurrentUser(created, "NOVIQWIKI_BACKUP_DIR");
  if ((created.mode & 0o077) !== 0) {
    throw new Error("NOVIQWIKI_BACKUP_DIR could not be created with private permissions.");
  }
  return canonical;
}
...
function assertDedicatedBackupPath(target: string) {
  ...
  if (
    ...
    throw new Error(
      "NOVIQWIKI_BACKUP_DIR must point to a dedicated backup directory, not a filesystem root, home directory, or workspace ancestor."
    );
  }
}
...
function assertDedicatedMediaPath(target: string) {
  ...
  if (
    ...
    throw new Error(
      "NOVIQWIKI_MEDIA_ROOT must point to a dedicated media directory, not a filesystem root, home directory, or workspace ancestor."
    );
  }
}
