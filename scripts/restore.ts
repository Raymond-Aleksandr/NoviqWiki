...
async function main() {
  ...
  if (mediaArchive && env.NOVIQWIKI_MEDIA_DRIVER !== "local") {
    throw new Error(
      "NOVIQWIKI_RESTORE_MEDIA can only restore the local media driver; restore S3 objects with your object-storage backup tooling."
    );
  }
  const composeDatabase = isComposeDatabaseTarget(env.DATABASE_URL);
  const composeMedia =
    Boolean(mediaArchive) &&
    usesComposeMediaTools(composeDatabase, env.NOVIQWIKI_MEDIA_DRIVER, env.NOVIQWIKI_MEDIA_ROOT);
  const confirmedMediaRoot = mediaArchive
    ? composeMedia
      ? composeMediaRoot
      : await assertSafeLocalMediaRoot(env.NOVIQWIKI_MEDIA_ROOT)
    : undefined;
  assertRestoreConfirmation(
    env.DATABASE_URL,
    process.env.NOVIQWIKI_RESTORE_CONFIRM,
    confirmedMediaRoot
  );
  ...
  try {
    ...
    if (requiresExplicitQuiescence && process.env.NOVIQWIKI_RESTORE_QUIESCED !== "true") {
      throw new Error(
        "Stop application writes and set NOVIQWIKI_RESTORE_QUIESCED=true before restoring a non-Compose database or local media path."
      );
    }
    if (composeDatabase) {
...
async function assertBoundLocalMediaRoot(confirmedMediaRoot: string) {
  ...
  if (currentMediaRoot !== confirmedMediaRoot) {
    throw new Error(
      "NOVIQWIKI_MEDIA_ROOT changed after restore confirmation; refusing to replace media."
    );
  }
