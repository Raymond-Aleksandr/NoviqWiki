...
async function main() {
  ...
  const backupDir = await prepareSafeBackupDirectory(process.env.NEXTWIKI_BACKUP_DIR ?? "backups");
  ...
  const composeMedia = usesComposeMediaTools(
    composeDatabase,
    env.NEXTWIKI_MEDIA_DRIVER,
    env.NEXTWIKI_MEDIA_ROOT
  );
  ...
  try {
    if (composeMedia) {
      ...
      env.NEXTWIKI_MEDIA_DRIVER === "local" &&
      process.env.NEXTWIKI_BACKUP_QUIESCED !== "true"
    ) {
      throw new Error(
        "Stop application writes and set NEXTWIKI_BACKUP_QUIESCED=true before backing up local media."
      );
    }

    if (env.NEXTWIKI_MEDIA_DRIVER === "local" && !composeMedia) {
      localMediaRoot = await assertSafeLocalMediaRoot(env.NEXTWIKI_MEDIA_ROOT);
