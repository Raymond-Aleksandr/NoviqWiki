...
Security also depends on deployment. Use a stable `NOVIQWIKI_SECRET`, HTTPS, restricted database and object-storage credentials, current dependencies, tested backups, and least-privilege roles. Session and CSRF Cookies become `Secure` only when the environment `NOVIQWIKI_BASE_URL` uses `https:`; configuring TLS or only the database-stored site URL is insufficient. Production requires `NOVIQWIKI_SECRET` to be explicitly configured with at least 32 characters, and Compose requires the same stable value before configuration can start. There is no generated fallback secret or secret volume. A database with a site but no active Owner exposes Owner bootstrap at `/setup`; complete it on a trusted network before public access. The v0.1.0 JSON resource API is same-origin only. Local and S3 media objects are authorized and streamed through the same-origin application route. Responses are publicly revalidated only when anonymous `media.read` is enabled; otherwise they use `private, no-store`. Review [Configuration](docs/CONFIGURATION.md), [Deployment](docs/DEPLOYMENT.md), [Authorization](docs/AUTHORIZATION.md), [API](docs/API.md), and [Backup and restore](docs/BACKUP_RESTORE.md).
...
安全性也取决于部署方式。请使用稳定的 `NOVIQWIKI_SECRET`、HTTPS、受限的数据库和对象存储凭据、最新依赖、经过验证的备份以及最小权限角色。只有环境变量 `NOVIQWIKI_BASE_URL` 使用 `https:` 时，会话和 CSRF Cookie 才带有 `Secure`；仅配置 TLS 或数据库中保存的站点 URL 并不足够。生产环境要求显式配置至少 32 个字符的 `NOVIQWIKI_SECRET`，Compose 也会在配置启动前要求同一个稳定值；不存在生成的回退密钥或密钥卷。已有站点但没有活跃 Owner 的数据库会在 `/setup` 暴露 Owner 引导，必须先在受信网络完成，再开放公共访问。v0.1.0 的 JSON 资源 API 仅支持同源使用。本地与 S3 媒体对象都通过同源应用路由授权并流式返回。只有匿名主体拥有 `media.read` 时响应才允许公开重新验证；其他情况使用 `private, no-store`。请阅读[配置](docs/CONFIGURATION.md)、[部署](docs/DEPLOYMENT.md)、[授权](docs/AUTHORIZATION.md)、[API](docs/API.md)和[备份与恢复](docs/BACKUP_RESTORE.md)。
