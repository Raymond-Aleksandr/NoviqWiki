...
Security also depends on deployment. Use a stable `NOVIQWIKI_SECRET`, HTTPS, restricted database and object-storage credentials, current dependencies, tested backups, and least-privilege roles. Session and CSRF Cookies become `Secure` only when the environment `NOVIQWIKI_BASE_URL` uses `https:`; configuring TLS or only the database-stored site URL is insufficient. Without an explicit `NOVIQWIKI_SECRET`, Compose persists a fallback in `noviqwiki-secrets`, which `pnpm backup` does not copy. Supplying an explicit value never writes it to that file and proactively deletes any old fallback; removing the explicit value later generates a new fallback and invalidates existing HMAC sessions and tokens. Production should use a stable, explicitly managed secret. A database with a site but zero users exposes Owner bootstrap at `/setup`; complete it on a trusted network before public access. The v0.1.0 JSON resource API is trusted-integration only. Private wikis must disable shared caching for the current local-media route; immediate revocation additionally requires `Cache-Control: private, no-store` plus purging cached objects or rotating previously distributed URLs. Review [Configuration](docs/CONFIGURATION.md), [Deployment](docs/DEPLOYMENT.md), [Authorization](docs/AUTHORIZATION.md), [API](docs/API.md), and [Backup and restore](docs/BACKUP_RESTORE.md).
...
安全性也取决于部署方式。请使用稳定的 `NOVIQWIKI_SECRET`、HTTPS、受限的数据库和对象存储凭据、最新依赖、经过验证的备份以及最小权限角色。只有环境变量 `NOVIQWIKI_BASE_URL` 使用 `https:` 时，会话和 CSRF Cookie 才带有 `Secure`；仅配置 TLS 或数据库中保存的站点 URL 并不足够。未显式提供 `NOVIQWIKI_SECRET` 时，Compose 会把回退密钥保存在 `noviqwiki-secrets` 中，而 `pnpm backup` 不会复制该卷。提供显式值时，启动流程绝不会将其写入该文件，并会主动删除任何旧回退文件；日后移除显式值会生成新的回退密钥，使现有 HMAC 会话和令牌失效。生产环境应使用稳定且由部署系统明确管理的密钥。已有站点但没有用户的数据库会在 `/setup` 暴露 Owner 引导，必须先在受信网络完成，再开放公共访问。v0.1.0 的 JSON 资源 API 仅限受信任集成。私有 Wiki 必须为当前本地媒体路由禁用共享缓存；若要求立即撤销访问，还必须配置 `Cache-Control: private, no-store`，并清除已缓存对象或轮换此前分发的 URL。请阅读[配置](docs/CONFIGURATION.md)、[部署](docs/DEPLOYMENT.md)、[授权](docs/AUTHORIZATION.md)、[API](docs/API.md)和[备份与恢复](docs/BACKUP_RESTORE.md)。
