...
- First-run setup wizard with initial `Owner` creation, plus an Owner-only bootstrap when a database already has a site but no users
...
Open <http://localhost:3000>. On a fresh database, the app directs you to the full setup wizard. A database with an existing site and zero users opens only the Owner-account steps; complete them before allowing untrusted access. Run `pnpm db:seed` only when you intentionally want the repository's preset development data.
...
- 首次运行初始化向导与初始 `Owner`（所有者）创建，以及数据库已有站点但没有用户时的仅 Owner 引导
...
打开 <http://localhost:3000>。全新数据库会进入完整初始化向导；已有站点且没有用户的数据库只显示 Owner 账号步骤，必须在允许不受信访问前完成。仅在确实需要仓库预设的开发数据时运行 `pnpm db:seed`。
...
显式设置 `UI_AUDIT_BASE_URL` 会指向上方 `pnpm dev`/`pnpm start` 使用的端口；`pnpm test:ui` 否则默认访问 `3100` 端口。它不会启动或重置应用；如果没有提供本地审核凭据，会跳过需要身份验证的路由。`pnpm test:e2e` 只能重置名称被识别为一次性用途的数据库。运行这两个命令前请阅读[测试指南](docs/TESTING.md)。
