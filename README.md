...
- First-run setup wizard with initial `Owner` creation, plus an Owner-only bootstrap when an existing site has no active Owner, even if non-Owner users remain
...
For a persistent installation, copy the environment template, set a strong database password, and generate separate signing and one-time setup secrets:
...
Keep `POSTGRES_PASSWORD` consistent with the percent-encoded password component in `DATABASE_URL`, set the canonical `NOVIQWIKI_BASE_URL`, and put separate generated values in `NOVIQWIKI_SECRET` and `NOVIQWIKI_SETUP_TOKEN`. Then start NoviqWiki:

```bash
docker compose up --build -d
```

Open <http://localhost:3000/setup>, enter `NOVIQWIKI_SETUP_TOKEN`, and complete the setup wizard. Remove the one-time setup token from the environment and restart after an active Owner is created. Production secrets are required and are never generated implicitly; Compose does not create a fallback signing secret or a secrets volume. Check container state with `docker compose ps` and logs with `docker compose logs --tail=200 app`.

The default Compose file keeps PostgreSQL on its private service network and binds the application only to `127.0.0.1:3000`. Host-based development can opt in to the loopback-only database mapping in `compose.dev.yaml`. Review [Configuration](docs/CONFIGURATION.md) and [Deployment](docs/DEPLOYMENT.md) before exposing an instance to the internet.

When PostgreSQL already contains a site but no active Owner remains, `/setup` switches to an Owner-only bootstrap and preserves the existing site settings, pages, media metadata, and non-Owner accounts. Public registration remains blocked until an active Owner exists. The first successful setup visitor can claim the Owner account, so complete this step on a trusted network before exposing such an instance.
...
Open <http://localhost:3000>. On a fresh database, the app directs you to the full setup wizard. An existing site without an active Owner opens only the Owner-account steps, even if non-Owner users remain; complete them before allowing untrusted access. Run `pnpm db:seed` only when you intentionally want the repository's preset development data.
...
pnpm test:integration  # Run PGlite and configured PostgreSQL integration tests
UI_AUDIT_BASE_URL=http://localhost:3000 pnpm test:ui  # Audit the local app above
pnpm test:e2e          # Run the disposable-database browser suite
```

Most integration cases use PGlite. Set `NOVIQWIKI_TEST_POSTGRES_URL` to a dedicated PostgreSQL 17 test database to run the PostgreSQL-specific migration and concurrency cases; the CI quality job supplies one. The explicit `UI_AUDIT_BASE_URL` targets the `pnpm dev`/`pnpm start` port shown above; `pnpm test:ui` otherwise defaults to port `3100`. It does not start or reset the app, and authenticated routes are skipped unless local audit credentials are supplied. `pnpm test:e2e` may reset only a database whose name is recognized as disposable. Read [Testing](docs/TESTING.md) before running either command.
...
- 首次运行初始化向导与初始 `Owner`（所有者）创建，以及现有站点没有 active Owner 时的仅 Owner 引导（即使仍有非 Owner 用户）
...
对于长期运行的实例，请复制环境变量模板，设置强数据库密码，并分别生成长期签名密钥和一次性设置令牌：
...
确保 `POSTGRES_PASSWORD` 与 `DATABASE_URL` 中经百分号编码的密码部分一致，设置规范的 `NOVIQWIKI_BASE_URL`，并为 `NOVIQWIKI_SECRET` 和 `NOVIQWIKI_SETUP_TOKEN` 分别填写生成值，然后启动 NoviqWiki：

```bash
docker compose up --build -d
```

打开 <http://localhost:3000/setup>，输入 `NOVIQWIKI_SETUP_TOKEN` 并完成初始化向导。active Owner 创建后，应从环境中移除该一次性令牌并重启服务。生产密钥为必填项，不会隐式生成；Compose 不会创建回退签名密钥或密钥卷。可使用 `docker compose ps` 检查容器状态，使用 `docker compose logs --tail=200 app` 查看日志。

默认 Compose 文件将 PostgreSQL 保持在私有服务网络中，并仅把应用绑定到 `127.0.0.1:3000`。主机开发可通过 `compose.dev.yaml` 显式启用仅回环地址的数据库端口映射。将实例暴露到互联网前，请阅读[配置](docs/CONFIGURATION.md)和[部署](docs/DEPLOYMENT.md)文档。

当 PostgreSQL 已有站点但没有 active Owner 时，`/setup` 会切换为仅创建 Owner 的引导，并保留现有站点设置、页面、媒体元数据和非 Owner 账号。active Owner 创建前，公开注册会保持阻断。第一个成功提交设置的人可以取得 Owner 账号，因此必须先在受信网络完成此步骤，再暴露该实例。
...
打开 <http://localhost:3000>。全新数据库会进入完整初始化向导；现有站点没有 active Owner 时只显示 Owner 账号步骤，即使仍有非 Owner 用户也一样，必须在允许不受信访问前完成。仅在确实需要仓库预设的开发数据时运行 `pnpm db:seed`。
...
pnpm test              # 运行单元测试
pnpm test:integration  # 运行 PGlite 和已配置的 PostgreSQL 集成测试
UI_AUDIT_BASE_URL=http://localhost:3000 pnpm test:ui  # 审核上方本地应用
pnpm test:e2e          # 运行使用一次性数据库的浏览器测试套件
```

大多数集成用例使用 PGlite。设置 `NOVIQWIKI_TEST_POSTGRES_URL` 指向专用 PostgreSQL 17 测试数据库后，会运行 PostgreSQL 专属的迁移与并发用例；CI 质量作业会提供该数据库。显式设置 `UI_AUDIT_BASE_URL` 会指向上方 `pnpm dev`/`pnpm start` 使用的端口；`pnpm test:ui` 否则默认访问 `3100` 端口。它不会启动或重置应用；如果没有提供本地审核凭据，会跳过需要身份验证的路由。`pnpm test:e2e` 只能重置名称被识别为一次性用途的数据库。运行这两个命令前请阅读[测试指南](docs/TESTING.md)。
