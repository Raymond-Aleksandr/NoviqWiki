...
export default async function OrphanedPages() {
  ...
  const [orphanedPages, i18n] = await Promise.all([
  ...
  return (
    <section className="page-frame">
      <nav className="breadcrumbs" aria-label={messages.breadcrumb}>
        ...
        <span>{messages.orphanedPages}</span>
      </nav>
      <header className="page-header">
        <div>
          <h1 className="page-title">{messages.orphanedPages}</h1>
          <p className="page-description">{messages.orphanedPagesDescription}</p>
        </div>
      ...
      <section className="data-panel">
        <div className="admin-panel-heading">{messages.orphanedPages}</div>
        {orphanedPages.length === 0 ? (
          <div className="empty-state backlinks-empty">
            <strong>{messages.noOrphanedPagesYet}</strong>
            <p className="muted">{messages.noOrphanedPagesBody}</p>
          </div>
        ) : (
