...
Do not add `-v` unless you intentionally want to delete the database, media, backup, and `nextwiki-secrets` volumes. `pnpm backup` does not copy the secrets volume, so retain the managed secret separately or include that volume in an infrastructure-level recovery plan.
...
Use `docker compose config --quiet` for a human gate. Plain `docker compose config` renders resolved environment values, including `NEXTWIKI_SECRET`, and must not be captured in logs or review artifacts.
...
The scheme of `NEXTWIKI_BASE_URL`, not `NODE_ENV`, controls the session and CSRF cookies' `Secure` attribute: an `https:` URL enables it and an `http:` URL disables it. Make the configured URL match the actual origin, and use HTTPS for production. Also keep `NEXTWIKI_SECRET` stable; changing it invalidates existing HMAC-backed sessions, email-verification tokens, and password-reset tokens.
...
未设置 `NEXTWIKI_SECRET` 或其值为空时，容器入口会从命名卷 `nextwiki-secrets` 复用 `/app/secrets/nextwiki-secret`；若该回退文件不存在，则生成它。回退值在容器重建和 `docker compose down` 后仍会保留。显式提供的环境值会直接使用，绝不会写入回退文件、`.env` 或 `.env.local`，并且启动时会主动删除任何旧回退文件。如果后来移除显式值，下次启动会生成新回退；密钥变化会使现有依赖 HMAC 的会话、电子邮件验证令牌和密码重置令牌失效。生产环境应显式从获批密钥管理器提供稳定密钥。
...
除非明确希望删除数据库、媒体、备份和 `nextwiki-secrets` 卷，否则不要添加 `-v`。`pnpm backup` 不会复制密钥卷，因此必须单独保留受管密钥，或把该卷纳入基础设施级恢复方案。
...
人类门禁应使用 `docker compose config --quiet`。普通 `docker compose config` 会渲染解析后的环境变量值，包括 `NEXTWIKI_SECRET`，不得把其输出保存到日志或审查制品中。
...
会话和 CSRF Cookie 的 `Secure` 属性由 `NEXTWIKI_BASE_URL` 的协议决定，而不是 `NODE_ENV`：`https:` URL 会启用它，`http:` URL 会禁用它。配置 URL 必须与实际来源一致，生产环境应使用 HTTPS。同时保持 `NEXTWIKI_SECRET` 稳定；更改它会使现有依赖 HMAC 的会话、电子邮件验证令牌和密码重置令牌失效。
