...
- 2026-07-16: Normalized `/admin/pages` filter control sizing after mobile review. The search field, status select, search button, clear-filter action, and create-page action now use the design-package compact control rhythm: 36px on desktop and 40px on mobile, with 16px mobile input/select text to prevent Safari focus zoom while preserving the compact layout.
